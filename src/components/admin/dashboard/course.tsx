...
export const Course  = () =>{
    const data = [
        {courseName: 'Nhập môn với lập trình hướng đối tượng',
         description: 'Giúp cho mọi người làm quen với lập trình hướng đối tượng',
          coverImage: '',
          language:'Tiếng Việt',
        },
    ...
    return(
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        {data.map((v) => (
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={v.courseName}>
              <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                {v.courseName}
              </th>
              <td className="px-6 py-4">{v.description}</td>
              <td className="px-6 py-4">{v.coverImage}</td>
              <td className="px-6 py-4">{v.language}</td>
              <td >
                <div className="m-px">
